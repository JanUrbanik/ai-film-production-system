# Adapter policy (factory law)

**Status:** v1.0 (2026-08-07)  
**Authority cross-ref:** Builder `07_Outputs/TOOL_GAP_AND_ADAPTER_POLICY.md`

1. **Default:** Grok Imagine only (image + video 1.5 workflows).  
2. **Adapters:** opt-in after failure class is named in QC log.  
3. **Never** run Kling+Veo+Seedance+Imagine in parallel by default.  
4. **Never** treat embedding scores or “92/100” as Imagine outputs.  
5. Showrunner or Human Director must approve adapter spend.  
6. Re-read `GROK_IMAGINE_CAPABILITY_MATRIX.md` before claiming new limits.  
